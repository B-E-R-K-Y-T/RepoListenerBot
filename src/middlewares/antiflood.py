...
class AntiFloodMiddleware(BaseMiddleware):
    def __init__(self, time_limit, message_limit, bot: AsyncTeleBot) -> None:
        self.last_time = {}
        self.message_count = {}
    ...
    async def pre_process(self, message, data):
        ...
        if message.date - self.last_time[user_id] < self.time_limit:
            ...
            self.last_time[user_id] = message.date
            self.message_count[user_id] = 1
            self.waiting_users.discard(
                user_id
            )  # Убираем пользователя из множества, так как он может снова писать
